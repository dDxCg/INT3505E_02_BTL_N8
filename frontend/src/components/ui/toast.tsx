...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={`fixed top-4 right-4 z-[9999] flex flex-col gap-2 w-full max-w-[420px] ${className || ''}`}
    {...props}
  />
));
...
>(({ className, variant = 'default', ...props }, ref) => {
  ...
  return (
    <ToastPrimitives.Root
      ref={ref}
      className={`group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-lg border-2 p-4 pr-8 shadow-lg ${
